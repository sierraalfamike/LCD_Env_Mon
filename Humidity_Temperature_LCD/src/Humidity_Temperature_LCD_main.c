...
dht11_data_t dht11_readout;
uint8_t*  dht11_readout_ptr;
...
void print_to_lcd()
{
	...
	sprintf(line0, "temperture: %d\xDF\C", temperature);
...
//	}
	...
	humidity_integer = 0;
	dht11_response_mask = 1;
	dht11_readout_ptr = (uint8_t*)&dht11_readout;
	...
	for ( i = 0; i < 5; i++ )
	{
		...
		for ( dht11_response_bit = 7; dht11_response_bit >= 0; dht11_response_bit-- )
		{
			...
			{
				P0_B3 = 1;
				humidity_integer |=  ( 1 << dht11_response_bit );
				delay_us(40);
				P0_B3 = 0;
			}
		}
		dht11_readout_ptr[i] = humidity_integer;
	}
	}
	humidity = dht11_readout.humidity_integer;
	temperature = dht11_readout.temperature_integer;
}
